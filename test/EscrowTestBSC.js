...
contract("StackEscrow", (accounts) => {
  ...
  describe("Setting platform fees", async () => {
    it("Set Variable Resource fees.", async () => {
      // 1% Param (Gov, Dao)
      await stackEscrow.setVariableFees("0", "100");
  ...
  describe("Developer buys STACK Tokens from Uni", async () => {
    ...
    it("SettleAccounts()", async () => {
      ...
      await stackEscrow
        ...
        .then(function (c) {
          ...
          assert.equal(totalDeposit == "20000000000000000000", true);
          assert.equal(totalDripRatePerSecond == "772090000000000", true);
        });
      ...
      await stackEscrow
        ...
        .then(function (c) {
          var totalDeposit = c["totalDeposit"];
          var totalDripRatePerSecond = c["totalDripRatePerSecond"];
          assert.equal(totalDeposit == "0", true);
          assert.equal(totalDripRatePerSecond == "0", true);
        });

      assert.equal(
        (await stackToken.balanceOf(daoAddress)) == "200000000000000000",
        true
      );
      assert.equal((await stackToken.balanceOf(govAddress)) == "0", true);
      assert.equal(
